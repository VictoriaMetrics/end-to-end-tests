...
COPY . .
# .terraform.lock.hcl is gitignored, so COPY . . above doesn't bring it into
# /app/terraform/gke. Copy the lock generated by the provider pre-install step
# so runtime `tofu init` resolves offline against TF_PLUGIN_CACHE_DIR instead
# of querying registry.terraform.io for version/checksum metadata every run.
RUN cp /terraform/gke/.terraform.lock.hcl terraform/gke/.terraform.lock.hcl
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    mkdir -p /tests && \
