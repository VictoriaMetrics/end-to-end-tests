...
resource "google_container_cluster" "primary" {
  ...
  cluster_autoscaling {
    ...
    resource_limits {
      resource_type = "cpu"
      minimum       = (var.min_node_count * 4) + (var.monitoring_min_node_count * 4)
      maximum       = (var.max_node_count * 4) + (var.monitoring_max_node_count * 4)
    }
    resource_limits {
      resource_type = "memory"
      minimum       = (var.min_node_count * 16) + (var.monitoring_min_node_count * 16)
      maximum       = (var.max_node_count * 16) + (var.monitoring_max_node_count * 16)
    }
